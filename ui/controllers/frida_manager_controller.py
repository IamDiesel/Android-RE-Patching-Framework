...
class FridaManagerController:
    ...
    def save_config(self, mode: str, host: str, port: str, sync_path: str):
        ...
        self.app.cfg.save()
        EventBus.publish("LOG_INFO", f"[*] Frida Config gespeichert. Modus: {mode.upper()}")
    ...
    def push_collection_to_device(self, collection_id: str, callback: Callable[[list], None]):
        col = next((c for c in self.manager.collections if c.id == collection_id), None)
        if not col: return
        ...
        def task():
            ...
            for script_id in col.script_ids:
                ...
                if compiled_path:
                    ...
                    os.replace(compiled_path, target_path)  # Fix für WinError 183
    ...
    def fire_usb_listen(self, script_code: str):
    ...
    def push_live_script(self, script_code: str):
    ...
    def push_to_device(self, script_code: str, filename: str, callback: Callable[[list], None]):
        if not filename.endswith(".js"): filename += ".js"
